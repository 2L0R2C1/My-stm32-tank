...
void MOTOR_Init(void){
	Bopan.encoder = htim1;
	Bopan.timepwm1 = htim2;
	Bopan.channelpwm1 = TIM_CHANNEL_1;
	Bopan.timepwm2 = htim2;
	Bopan.channelpwm2 = TIM_CHANNEL_2;
	Bopan.capturetim = htim4;
...
void MOTOR_Tim_Init(MOTOR_TypeDef *motor){	
	...
	__HAL_TIM_CLEAR_IT(&motor->capturetim,TIM_IT_UPDATE);	//清零tim4中断标志位(定时中断获取电机状态并反馈）
	__HAL_TIM_SetCounter(&motor->capturetim,0);				//归零tim4计数值
	HAL_TIM_Base_Start_IT(&motor->capturetim);				//开启tim4计数中断	
}
...
//	memset(motor,0,sizeof(Bopan));
	...
	HAL_TIM_Base_Stop_IT(&motor->capturetim);				//关闭tim4计数中断	
//	__HAL_TIM_CLEAR_IT(&motor->capturetim,TIM_IT_UPDATE);	//清零tim4中断标志位(定时中断获取电机状态并反馈）
//	__HAL_TIM_SetCounter(&motor->capturetim,0);				//归零tim4计数值
