...
void vision_control(u8 order){
	...
	xx=0.0f,yy=0.0f;
	while(q<m)xx += xp[q], yy += yp[q], q++;	//均值处理，使移动平滑，减少摆动
	xx = xx*70/(float)m; yy = yy/(float)m;
//	xx=x; yy=y;	
//	printf("xx=%f yy=%f\r\n",xx,yy);

	Forward_L.target_angle += xx;    //这里只是改变目标值，而实际值有一定变化过程（电机pid），及有一定的响应时间，避免突变产生整车震动影响视觉
	Forward_R.target_angle -= xx;
	Back_L.target_angle += xx,	
	Back_R.target_angle -= xx;
	if(0<=Turret.angle+yy&&Turret.angle+yy<=50)Turret.angle += yy;//只是改变舵机目标角度，底层代码是让舵机匀速转到目标角度，减少震动
}
```
